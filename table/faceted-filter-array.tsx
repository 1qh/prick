...
export function DataTableFacetedFilterContainArray<TData, TValue>({
  ...
  for (const option of options) {
    const count = [...(_facets?.keys() as any)]
      ?.filter(key => key.includes(option))
      .map(key => _facets?.get(key))
      .reduce((acc, value) => (acc as number) + (value as number), 0)
    facets.set(option, count as number)
  }
  options = options.sort((a, b) => {
    return (facets.get(b) as number) - (facets.get(a) as number)
  })
